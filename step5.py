...
ax2.imshow(cv2.cvtColor(get_frame(103), cv2.COLOR_BGR2RGB))
...
img,joints=get_joints(get_frame(100))
plt.figure()
...
#        M_batch[label,...]=part_M
        img_new = mask + cv2.warpAffine(part_mask,M[label,...],(W,H))
...
Frame_ref= get_frame(103)
H_in=get_bodymap(Frame_in)
H_out=get_bodymap(Frame_ref)
t,Pose_in=get_joints(Frame_in)
t,Pose_ref=get_joints(Frame_ref)
#print(Pose_in)
...
res_img=draw_pose(New_pose_in,main_skeleton_lines)
plt.figure()
plt.imshow(res_img)
...
M=get_M_matirx(New_pose_in,New_pose_ref,main_skeleton_lines)
img_transfer=body_transfer(Frame_in,H_in,M)
...
H_out=old2new_mask(H_out,New_pose_ref,main_skeleton_lines)
plt.imshow(H_out)
fn='./output/res_step3/'+str(7)+'.png'
plt.savefig(fn)
plt.close()