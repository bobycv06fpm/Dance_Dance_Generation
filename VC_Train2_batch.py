...
def train(epochs):
  for epoch in range(epochs):
    start = time.time()
    #idx_shu=np.random.choice(BUFFER_SIZE, int(BUFFER_SIZE/BATCH_SIZE) , replace=False)
    for tmp_idx in range(0,int(BUFFER_SIZE/BATCH_SIZE)):
    ...
    tinp=inp[0,...]
    ttar=tar[0,...]
    generate_images(generator, tinp[tf.newaxis,...], ttar[tf.newaxis,...])
